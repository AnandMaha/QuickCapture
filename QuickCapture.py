...
import json
import ast # for incoming list input
import fileinput
import spacy
...
start_befores = ["from","on",'beginning on', 'commencing', 'starting from', "starting",'inaugurating', 'initiating', 'launching on', 'opening on', 'originating', 'setting out', 'starting off', 'undertaking', 'embarking', 'kickstarting', 'triggering on', \
    'genesis on', 'outset', 'onset', 'introduction', 'premiere','from','at',"beginning","in"]
...
def quickCapture(s):
    ...
    #print("Entities [TEXT, ENTITY]:",[(ent.text, ent.label_) for ent in doc.ents])
    ...
    #display(start_date.strftime("%c"), end_date.strftime("%c"), frequency)
    ...
    for ent in doc.ents:
        ...
        #print("asdf",ent.start_char,ent.start_char > 0 and before_ent.endswith(tuple(start_befores)))
        if ent.label_ == "occurrences":
            occurrences = int(w2n.word_to_num(ent.text.split()[0]))
        if ent.label_ == "DATE" or ent.label_ == "TIME" or ent.label_ == "DateTime":
            ...
            if freq_list:
                ...
                #print("end date:",ent.text,"before word:",before_ent)
                no_end_set = False
                ...
                if ent.start_char > 0 and start_list:
                    ...
                    #print("start date:",ent.text,"before word:",before_ent)
                else:
                    task = task.replace(ent.text, "")
    ...
    # options = {"ents": None,"colors": {"DATE": "cyan"}}
